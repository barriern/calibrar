...
.ahres = function(par, fn, gr, lower, upper, control, method, hessian) {
  ...
  # Init the proc rank to 0
  .comm.rank = 0

  # If the parallel mode is pbdMPI, then we
  # set the .comm.rank to the MPI rankl=
  if(!is.null(control$parallel_mode)) {
    if(control$parallel_mode == "pbdMPI") {
        .comm.rank = comm.rank()
    }
  }
  ...
  if(isTRUE(restart)) {
    ...
    if(control$REPORT>0 & control$trace>0 & (.comm.rank == 0)) {
  ...
  if (.comm.rank == 0) copy_master_folder(control, n=control$popsize)
  ...
  while(isTRUE(cc <- .continueEvolution(opt, control))) {
    ...
    # from rank 0 to all other ranks. And all ranks receive population
    # values from rank 0
    if(control$parallel_mode == "pbdMPI") {

        # If we are rank 0, we send population generated by
        # rank 0 to all procs
        if(.comm.rank == 0) {
            for(irank in 1:(comm.size() - 1)) {
                send(opt$pop, irank, tag=(irank + 1) * 1000)
            }
        } else {
